...
namespace UWin32
{
	...
	{
		[PreserveSig]
		int Next(int celt, out System.Runtime.InteropServices.ComTypes.STATSTG sstg, IntPtr celtOut);
	};
...
//	Guid("
	...
	public interface UCOMIStorage
	{
		void CreateStream(
			...
			System.Runtime.InteropServices.ComTypes.IStream stream //Pointer to output variable
			);

		System.Runtime.InteropServices.ComTypes.IStream OpenStream(
			[MarshalAs(UnmanagedType.LPWStr)] string wcsName,
...
//			out UCOMIEnumSTATSTG iEnum);
		...
		void RenameElement(
			[MarshalAs(UnmanagedType.LPWStr)] string wcsOld,
			[MarshalAs(UnmanagedType.LPWStr)] string wcsNew);
		...
		void SetClass(
			[MarshalAs(UnmanagedType.Struct)] ref Guid clsid);

		void SetStateBits(
			Int32 grfStateBits,
			Int32 grfMask);

		void Stat(out System.Runtime.InteropServices.ComTypes.STATSTG sstg, Int32 grfStatFlag);
